...
def save_img_matrix(filename, run_query_ids, imgs_seen_ops, input_folder, pred_folder, label_folder, save_dir = None, seed = 44, iter_num = 0):
    ...
    fig, ax = plt.subplots(len(run_query_ids),len(imgs_seen_ops)+2, figsize = (16,4), squeeze = False)
    ...
    for r in range(len(run_query_ids)):
        ...
        ax[r, -1].imshow(sitk.GetArrayFromImage(sitk.ReadImage(label_path))[0])
        ax[r, -1].axis('off')

        img_path = os.path.join(input_folder, filename[:-7]+'_0000.nii.gz')
        ax[r, 0].imshow(sitk.GetArrayFromImage(sitk.ReadImage(img_path))[0])
        ax[r, 0].set_ylabel(run_query_ids[r],rotation=90, labelpad=20, fontsize = 5)
        ax[r,0].set_xticks([])
        ax[r,0].set_yticks([])
        # ax[r, 0].axis('off')
